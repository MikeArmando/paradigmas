...
void simularImpresion(Queue *queue);
...
void menu(void)
{
    ...
    do
    {
        ...
        case 6:
            ...
            break;
        case 7:
            simularImpresion(&queue);
            pause("Proceso terminado. Todos los trabajos fueron procesados.");
            break;
        case 0:
            limpiarQueue(&queue);
...
int msg()
{
    ...
    printf("7.- SIMULAR IMPRESION DE TODA LA COLA. \n");
    printf("0.- SALIR.  \n");
...
void inQueue(Queue *queue, Trabajo trabajo)
{
    ...
    {
        if (trabajo.prioridad == 2)
        {
            nuevoNodo->siguiente = queue->frente;
            queue->frente = nuevoNodo;
            printf("Trabajo urgente agregado al inicio de la cola.\n");
        }
        else
        {
...
void deQueue(Queue *queue)
{
    ...
    Trabajo trabajo = actual->dato;
    queue->frente = queue->frente->siguiente;

    int totalImpresiones = trabajo.paginasTotales * trabajo.copias;
    int realizadas = 0;

    printf("\n----- INICIANDO IMPRESION -----\n");
    printf("Usuario: %s | Total: %d impresiones\n", trabajo.usuario, totalImpresiones);

    while (realizadas < totalImpresiones)
    {
        realizadas++;
        float avance = ((float)realizadas / totalImpresiones) * 100;

        int copiaActual = ((realizadas - 1) / trabajo.paginasTotales) + 1;
        int pagActual = ((realizadas - 1) % trabajo.paginasTotales) + 1;

        printf("\r   -> [IMPRIMIENDO] Copia %d/%d - Pag %d/%d | Progreso: %.1f%% ",
               copiaActual, trabajo.copias, pagActual, trabajo.paginasTotales, avance);
...
void printQueue(Queue *queue)
{
    ...
    printf("| %5s | %15s | %15s | %15s | %15s | %15s | %15s | %15s |\n", "ID",
           "USUARIO", "DOMICILIO", "PAG. TOTALES", "PAG. RESTANTES", "COPIAS", "PRIORIDAD", "ESTADO");
    while (actual != NULL)
    {
        printf("| %5d | %15s | %15s | %15d | %15d | %15d | %15s | %15s |\n",
...
void simularImpresion(Queue *queue)
{
    if (queue->cant == 0)
    {
        pause("La cola esta vacia. No hay nada que imprimir.");
        return;
    }
    ...
    pause("Todos los trabajos han sido completados.");
}
